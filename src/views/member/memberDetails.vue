    ...
    <div class="v2-details">
        <div class="items p15">
            ...
            <div class="remarks ti20 bl br bb clearfix">
                ...
                <Button  size="small" class="moreBtn" v-if="basicInfo.remark_self&&basicInfo.remark_self.length>0||basicInfo.emark_trigger&&basicInfo.emark_trigger.length>0"  @click="onMore">更多...</Button>
        ...
        <div class="items mt15">
         ...
         <Modals title="备注详情" ref="remarkModal" width='1100' height="400" >
             <div slot="content" class="pl20 pr20 pt20 pb20">
                 <span class="ft16 font-bold" v-if="basicInfo.remark_self&&basicInfo.remark_self.length>0" >备注：</span>
                 <p  class="ft15" style="word-break:break-all;word-wrap:break-word;white-space:normal" v-for="(item,i) in  remark_self " :key="i" >{{i+1}}.{{item}}</p>
             <span class="ft16 mt5 mb5 font-bold"  v-if="basicInfo.remark_trigger&&basicInfo.remark_trigger.length>0" >触发用户备注：</span>
             <div class="ft15" v-for="(t,i) in  remark_trigger " :key="i">
                 <p><span class="mb5">站点名称：{{t.siteName}}</span> <span class="ml5">用户名：{{t.username}}</span></p>
                 <div style="word-break:break-all;word-wrap:break-word;white-space:normal" v-for="(t,i) in t.list" :key="i" >{{i+1}}. <span style="word-wrap:break-word" >{{t}}</span></div>
             <div style="width:95%;border-bottom: 1px solid;" class="mt10 mb10"></div></div>
             </div>
         </Modals>
...
export default {
    ...
    methods: {
        onMore(){
            ...
            //   console.log(this.remark_self.length)
